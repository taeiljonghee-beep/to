...
export default function PoolGuide() {
  return (
    <main className="pt-16 bg-[#F9F8F4]">
      ...
      <section className="px-6 lg:px-10 pb-20 max-w-[1400px] mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {[
            { label: '이용 시간', color: '#85C4DC', items: ['수영장 이용 가능 시간: [시간 입력]', '야간 이용 시 안전에 유의해 주세요.'] },
            { label: '안전 수칙', color: '#EF8C72', items: ['12세 미만은 반드시 보호자 동반 입수', '음식 섭취 후 30분 후 입수', '달리기 금지', '다이빙 금지'] },
            { label: '이용 규칙', color: '#C2B0E8', items: ['수영복 필수 착용', '유리 용기 반입 금지', '음식물 반입 금지', '애완동물 입수 금지'] },
            { label: '구비 용품', color: '#8ECFC0', items: ['[구비 용품 목록 입력]', '수영 용품 대여 여부: [입력]'] },
          ].map((sec) => (
